...
// التحقق من تسجيل الدخول كمدرّس أو أدمن
if (!current_user()) {
...
if (!in_array($user['role'], ['admin', 'instructor'])) {
    ...
    echo '<h1>غير مسموح</h1><p>هذه الصفحة مخصصة للمدرسين والأدمن فقط.</p>';
...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if (!verify_csrf_token($token)) {
        $errorMessage = 'خطأ أمني: يرجى تحديث الصفحة والمحاولة مرة أخرى';
    } else {
    try {
        $pdo = getPDO();
        ...
        header('Location: ../admin/dashboard.php?msg=uploaded&lessons=' . $uploadedLessons);
        exit;
...
<nav class="navbar navbar-expand-lg navbar-light bg-white shadow-sm">
  <div class="container">
    ...
    <div class="ms-auto">
      <a class="btn btn-secondary me-2" href="../admin/dashboard.php">
